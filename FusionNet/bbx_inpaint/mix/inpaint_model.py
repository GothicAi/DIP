...
from inpaint_ops import traditional_inpaint
...
class InpaintCAModel(Model):
    ...
    def build_inpaint_net(self, x, mask, config=None, reuse=False,
        ...
        with tf.variable_scope(name, reuse=reuse), \
            ...
            x = gen_conv(x, 1, 3, 1, activation=None, name='weight_conv')
            x = tf.clip_by_value(x, -1., 1.)
            mix_weight = (x+1.0)/2.0
        return x_stage1, x_stage2, offset_flow, mix_weight
    ...
    def build_graph_with_losses(self, batch_data, config, training=True,
        ...
        batch_pos = batch_data / 127.5 - 1. #(16, 256, 256, 3)
        # generate mask, 1 represents masked point
        bbox = random_bbox(config)
        mask = bbox2mask(bbox, config, name='mask_c') #(1, 256, 256, 1)
        traditional_batch = traditional_inpaint(batch_data, mask)
        batch_incomplete = batch_pos*(1.-mask)
        x1, x2, offset_flow, mix_weight = self.build_inpaint_net(
            batch_incomplete, mask, config, reuse=reuse, training=training,
            padding=config.PADDING)
        # x2 (16, 256, 256, 3) clip(-1,1)
        x2 = traditional_batch*(1.0-mix_weight) + x2*mix_weight
    ...
    def build_server_graph(self, batch_data, reuse=False, is_training=False):
        ...
        traditional_batch = traditional_inpaint(batch_data, masks)
        batch_incomplete = batch_pos * (1. - masks)
        # inpaint
        x1, x2, flow, mix_weight = self.build_inpaint_net(
