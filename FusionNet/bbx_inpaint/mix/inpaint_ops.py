...
def traditional_inpaint(img_batch, mask, name='traditional'):
    def inpaint_batch(img_batch, mask):
        mymask = np.array(mask[0], dtype='uint8').reshape(256,256)*255
        dst_batch = None
        for i in range(len(img_batch)):
            img = np.array(img_batch[i], dtype='uint8')
            dst = cv2.inpaint(img, mymask, 3, cv2.INPAINT_NS)
            dst = np.array(dst, dtype='float32') / 127.5 - 1
            if i:
                dst_batch = np.append(dst_batch, dst, axis=0)
            else:
                dst_batch = dst
        # dst_batch = dst_batch.reshape(16, 256, 256, 3)
        dst_batch = dst_batch.reshape(1, 400, 400, 1)
        return dst_batch
    with tf.variable_scope(name), tf.device('/cpu:0'):
        traditional_batch = tf.py_func(
            inpaint_batch,
            [img_batch, mask],
            tf.float32, stateful=True)
        # traditional_batch.set_shape([16, 256, 256, 3])
        traditional_batch.set_shape([1, 400, 400, 1])
    return traditional_batch
