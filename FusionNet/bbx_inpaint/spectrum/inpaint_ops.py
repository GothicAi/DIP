...
def conv(x, channels, kernel=4, stride=2, scope='conv_0', activation=tf.nn.elu, padding='SAME'):
    with tf.variable_scope(scope):
        w = tf.get_variable("kernel", shape=[kernel, kernel, x.get_shape()[-1], channels], initializer=weight_init,
                            regularizer=weight_regularizer)
        x = tf.nn.conv2d(input=x, filter=spectral_norm(w),
                            strides=[1, stride, stride, 1], padding=padding)
        if activation == tf.nn.elu:
            x = tf.nn.elu(x)
...
def gen_conv(x, cnum, ksize, stride=1, rate=1, name='conv',
    ...
    if padding == 'SYMMETRIC' or padding == 'REFELECT':
        ...
        padding = 'VALID'
    if rate == 1:
        x = conv(x, cnum, ksize, stride, scope=name, activation=activation, padding=padding)
    else:
        x = tf.layers.conv2d(
...
def dis_conv(x, cnum, ksize=5, stride=2, name='conv', training=True):
    ...
    """
    with tf.variable_scope(name):
        x = conv(x, cnum, ksize, stride, scope=name, activation=None, padding='SAME')
