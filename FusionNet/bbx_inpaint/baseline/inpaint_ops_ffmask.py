...
def random_bbox(config):
    ...
    return (tf.constant(0), tf.constant(0), h, w)
def bbox2mask(bbox, config, name='mask'):
    from PIL import Image
    import os
    def npmask(bbox, height, width, delta_h, delta_w):
        mask = np.zeros((1, height, width, 1), np.float32)
        maskDir = '/BigDisk/jianhua/dataset/maskData/'
        maskList = os.listdir(maskDir)
        name = np.random.choice(maskList)
        mask_pre = Image.open(maskDir+name).point(lambda i : 0 if i==0 else 1)
        mask_pre = np.array(mask_pre)
        mask[0,:,:,0] = mask_pre.astype(np.float32)
        return mask
