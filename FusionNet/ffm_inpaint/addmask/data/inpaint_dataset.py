...
class InpaintDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        for mask_type in self.mask_paths:
            mask_paths[mask_type] = self.mask_paths[mask_type][index%100]
    ...
    def read_mask(self, path, mask_type):
        ...
        if mask_type == 'random_bbox':
            ...
            choice = np.random.choice(['ff', 'character', 'obj'], p=[0.7, 0.1, 0.2])
            if choice == 'ff':
                mask = InpaintDataset.random_ff_mask(self.random_ff_setting)
            elif choice == 'character':
                maskdir = '/data/jianhua/dataset/maskData/charactermask/'
                masklist = os.listdir(maskdir)
                maskname = np.random.choice(masklist)
                mask = cv2.imread(maskdir+maskname, 0)/255
                mask = cv2.resize(mask,(256,256),interpolation=cv2.INTER_CUBIC)
                mask = np.transpose([mask],(1,2,0))
            elif choice == 'obj':
                maskdir = '/data/jianhua/dataset/maskData/objmask/'
                masklist = os.listdir(maskdir)
                maskname = np.random.choice(masklist)
                mask = cv2.imread(maskdir+maskname, 0)/255
                mask = cv2.resize(mask,(256,256),interpolation=cv2.INTER_CUBIC)
